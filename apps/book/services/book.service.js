...
function getGBooks(query) {
    const api = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${query}&fields=items(id,volumeInfo/title)`
    return axios.get(api).then((res) => {
        return res.data.items
    })
}

function addGBook(gBookId) {
    const api = `https://www.googleapis.com/books/v1/volumes/${gBookId}`
    return axios.get(api).then((res) => {
        return res.data
    }).then(gBook => {
        const { title, subtitle, authors, publishedDate, description, pageCount, categories, imageLinks, language } = gBook.volumeInfo
        const book = {
            id: gBook.id,
            title,
            subtitle,
...
function getNextId(bookId, prevIdx = false) {
    let books = _loadFromStorage()
    const bookIdx = books.findIndex(book => book.id === bookId)
    const nextIdx = (prevIdx) ?
        (bookIdx === 0) ? books.length - 1 : bookIdx - 1 :
        (bookIdx + 1 === books.length) ? 0 : bookIdx + 1
    return books[nextIdx].id
}

function reviewAdd(bookToUpdate) {
    let books = _loadFromStorage()
